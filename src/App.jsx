...
function CompanyListView({ followedCompanies, onOpenDetail, onFollowChange }) {
  ...
  const [alertRules, setAlertRules] = useState([
    { id: 1, companyId: "huaneng", metric: "范围1+2排放强度", operator: "高于", threshold: "45", unit: "tCO₂e/MWh", frequency: "每月检查", enabled: true },
    { id: 2, companyId: "byd", metric: "范围1+2排放总量", operator: "同比增长超过", threshold: "10", unit: "%", frequency: "披露更新时", enabled: true },
  ]);
  ...
  function addAlertRule(event) { event.preventDefault(); if (!alertForm.companyIds.length || alertForm.metrics.some((item) => !item.threshold)) return; const now = Date.now(); const newRules = alertForm.companyIds.flatMap((companyId, companyIndex) => alertForm.metrics.map((metric, metricIndex) => ({ ...metric, companyId, frequency: alertForm.frequency, id: now + companyIndex * 100 + metricIndex, enabled: true }))); setAlertRules((rules) => [...rules, ...newRules]); setAlertForm((form) => ({ ...form, metrics: form.metrics.map((item) => ({ ...item, threshold: "" })) })); setAlertSettingsMode("list"); }
