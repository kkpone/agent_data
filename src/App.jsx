...
function CompanyListView({ followedCompanies, onOpenDetail, onFollowChange }) {
  ...
  const [alertForm, setAlertForm] = useState({ companyId: followedCompanies[0]?.id || "", metric: "范围1+2排放强度", operator: "高于", threshold: "", unit: "tCO₂e/MWh", frequency: "每月检查" });
  const alertMessages = [
  ...
  function updateAlertMetric(metric) { const option = alertMetrics.find((item) => item.label === metric); setAlertForm((form) => ({ ...form, metric, unit: option?.unit || "" })); }
  function addAlertRule(event) { event.preventDefault(); if (!alertForm.companyId || !alertForm.threshold) return; setAlertRules((rules) => [...rules, { ...alertForm, id: Date.now(), enabled: true }]); setAlertForm((form) => ({ ...form, threshold: "" })); }
