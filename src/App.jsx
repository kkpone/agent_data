...
import {
  ...
  IconMessagePlus, IconNetwork, IconPaperclip, IconPencil, IconReportAnalytics, IconRoute, IconBell, IconDots, IconPlus,
...
function CompanyListView({ followedCompanies, onOpenDetail, onFollowChange }) {
  ...
  const [alertTab, setAlertTab] = useState("messages");
  const [alertSettingsMode, setAlertSettingsMode] = useState("list");
  const [editingAlertRuleId, setEditingAlertRuleId] = useState(null);
  const [alertRules, setAlertRules] = useState([
    { id: 1, name: "重点企业排放强度预警", companyIds: ["huaneng", "byd"], metric: "范围1+2排放强度", operator: "高于", threshold: "45", unit: "tCO₂e/MWh", frequency: "每月检查", enabled: true },
    { id: 2, name: "年度排放增长监测", companyIds: ["huaneng", "byd", "longi"], metric: "范围1+2排放总量", operator: "同比增长超过", threshold: "10", unit: "%", frequency: "披露更新时", enabled: true },
  ]);
  const [alertForm, setAlertForm] = useState({ name: "", companyIds: followedCompanies[0] ? [followedCompanies[0].id] : [], metrics: [{ metric: "范围1+2排放强度", operator: "高于", threshold: "", unit: "tCO₂e/MWh" }], frequency: "每月检查" });
  const alertMessages = [
    { id: 1, companyId: "huaneng", title: "排放强度超过设定阈值", detail: "范围1+2排放强度为 49.6 tCO₂e/MWh，高于阈值 45 tCO₂e/MWh", time: "今天 09:32", unread: true },
    { id: 2, companyId: "byd", title: "年度披露数据已更新", detail: "范围1+2排放总量同比增长 12.4%，超过阈值 10%", time: "昨天 16:18", unread: true },
    { id: 3, companyId: "longi", title: "企业碳数据更新", detail: "2025 年范围2市场法排放数据已发布，可前往档案查看", time: "09月18日", unread: false },
  ];
  ...
  const alertMetrics = [{ label: "范围1排放量", unit: "tCO₂e" }, { label: "范围2排放量（市场法）", unit: "tCO₂e" }, { label: "范围1+2排放总量", unit: "tCO₂e" }, { label: "范围1+2排放强度", unit: "tCO₂e/MWh" }, { label: "单位营收碳排放强度", unit: "tCO₂e/百万元" }];
  function toggleAlertCompany(id) { setAlertForm((form) => ({ ...form, companyIds: form.companyIds.includes(id) ? form.companyIds.filter((item) => item !== id) : [...form.companyIds, id] })); }
  function updateAlertMetric(index, field, value) { setAlertForm((form) => ({ ...form, metrics: form.metrics.map((item, itemIndex) => { if (itemIndex !== index) return item; if (field === "metric") { const option = alertMetrics.find((metric) => metric.label === value); return { ...item, metric: value, unit: option?.unit || "" }; } return { ...item, [field]: value }; }) })); }
  function addAlertMetric() { const used = alertForm.metrics.map((item) => item.metric); const option = alertMetrics.find((item) => !used.includes(item.label)) || alertMetrics[0]; setAlertForm((form) => ({ ...form, metrics: [...form.metrics, { metric: option.label, operator: "高于", threshold: "", unit: option.unit }] })); }
  function openNewAlertRule() { setEditingAlertRuleId(null); setAlertForm((form) => ({ ...form, name: "", metrics: [{ metric: "范围1+2排放强度", operator: "高于", threshold: "", unit: "tCO₂e/MWh" }] })); setAlertSettingsMode("form"); }
  function editAlertRule(rule) { setEditingAlertRuleId(rule.id); setAlertForm({ name: rule.name, companyIds: [...rule.companyIds], metrics: [{ metric: rule.metric, operator: rule.operator, threshold: rule.threshold, unit: rule.unit }], frequency: rule.frequency }); setAlertSettingsMode("form"); }
  function saveAlertRule(event) { event.preventDefault(); if (!alertForm.name.trim() || !alertForm.companyIds.length || alertForm.metrics.some((item) => !item.threshold)) return; if (editingAlertRuleId !== null) { const metric = alertForm.metrics[0]; setAlertRules((rules) => rules.map((rule) => rule.id === editingAlertRuleId ? { ...rule, ...metric, name: alertForm.name.trim(), companyIds: [...alertForm.companyIds], frequency: alertForm.frequency } : rule)); } else { const now = Date.now(); const newRules = alertForm.metrics.map((metric, metricIndex) => ({ ...metric, name: alertForm.metrics.length > 1 ? `${alertForm.name.trim()}-${metricIndex + 1}` : alertForm.name.trim(), companyIds: [...alertForm.companyIds], frequency: alertForm.frequency, id: now + metricIndex, enabled: true })); setAlertRules((rules) => [...rules, ...newRules]); } setEditingAlertRuleId(null); setAlertSettingsMode("list"); }
  function companyStockType(company) { if (company.code.includes("/") && company.code.includes("HK")) return "A+H股"; if (company.code.includes(".SH") || company.code.includes(".SZ")) return "A股"; if (company.code.includes("HK")) return "港股"; return "非上市"; }
  ...
  const alertRuleLayout = <div className={`alert-rule-layout page-layout ${alertSettingsMode === "form" || !alertRules.length ? "form-only" : "list-only"}`}>
    <section className="alert-rule-list alert-rule-table"><div className="alert-section-title"><strong>已设置规则</strong><span>{alertRules.length} 条</span></div>{alertRules.length ? <div className="alert-table"><div className="alert-table-head"><span>名称</span><span>监测企业</span><span>规则</span><span>是否开启</span><span>操作</span></div>{alertRules.map((rule) => { const ruleCompanies = companies.filter((item) => rule.companyIds.includes(item.id)); return <article className={!rule.enabled ? "disabled" : ""} key={rule.id}><strong>{rule.name}</strong><div className="alert-rule-companies">{ruleCompanies.map((company) => <em key={company.id}>{company.short}</em>)}</div><div className="alert-rule-condition"><b>{rule.metric}</b><span>{rule.operator} {rule.threshold} {rule.unit}</span><small>{rule.frequency} · 站内提醒</small></div><button className={`alert-switch ${rule.enabled ? "on" : ""}`} onClick={() => setAlertRules((rules) => rules.map((item) => item.id === rule.id ? { ...item, enabled: !item.enabled } : item))} aria-label={rule.enabled ? "停用提醒" : "启用提醒"}><i /></button><div className="alert-row-actions"><button className="alert-edit" onClick={() => editAlertRule(rule)} aria-label="修改提醒"><IconPencil size={14} />编辑</button><button className="alert-delete" onClick={() => setAlertRules((rules) => rules.filter((item) => item.id !== rule.id))} aria-label="删除提醒"><IconX size={14} />删除</button></div></article>; })}</div> : <div className="alert-rule-empty"><IconBell size={26} /><span>暂无提醒规则</span></div>}</section>
    <form className="alert-rule-form" onSubmit={saveAlertRule}><div className="alert-section-title"><strong>{editingAlertRuleId !== null ? "修改规则" : "新建提醒"}</strong><span>站内通知</span></div><label className="alert-rule-name">阈值规则名称<input autoFocus value={alertForm.name} onChange={(event) => setAlertForm((form) => ({ ...form, name: event.target.value }))} placeholder="请输入便于识别的规则名称" maxLength={30} /></label><fieldset className="alert-company-picker"><legend>监测企业 <span>已选 {alertForm.companyIds.length} 家</span></legend><details className="alert-company-multiselect"><summary><span>{alertForm.companyIds.length ? followedCompanies.filter((company) => alertForm.companyIds.includes(company.id)).map((company) => company.short).join("、") : "请选择监测企业"}</span><IconChevronDown size={15} /></summary><div>{followedCompanies.map((company) => { const checked = alertForm.companyIds.includes(company.id); return <button type="button" className={checked ? "selected" : ""} key={company.id} onClick={() => toggleAlertCompany(company.id)}><i>{checked ? <IconCircleCheck size={13} /> : company.short.slice(0,1)}</i><span><strong>{company.name}</strong><small>{company.code}</small></span></button>; })}</div></details></fieldset><fieldset className="alert-metric-builder"><legend>监测指标 <span>{editingAlertRuleId !== null ? "修改指标触发条件" : "每项单独配置触发条件"}</span></legend>{alertForm.metrics.map((metric, index) => <article key={`${metric.metric}-${index}`}><div className="metric-builder-heading"><b>指标 {index + 1}</b>{editingAlertRuleId === null && alertForm.metrics.length > 1 && <button type="button" onClick={() => setAlertForm((form) => ({ ...form, metrics: form.metrics.filter((_, itemIndex) => itemIndex !== index) }))}><IconX size={14} /></button>}</div><label>指标<select value={metric.metric} onChange={(event) => updateAlertMetric(index, "metric", event.target.value)}>{alertMetrics.map((item) => <option key={item.label}>{item.label}</option>)}</select></label><div className="alert-condition-row"><label>触发条件<select value={metric.operator} onChange={(event) => updateAlertMetric(index, "operator", event.target.value)}><option>高于</option><option>低于</option><option>同比增长超过</option><option>同比下降超过</option></select></label><label>阈值<div className="alert-threshold-input"><input type="number" min="0" step="0.1" value={metric.threshold} onChange={(event) => updateAlertMetric(index, "threshold", event.target.value)} placeholder="请输入" /><span>{metric.operator.includes("同比") ? "%" : metric.unit}</span></div></label></div></article>)}{editingAlertRuleId === null && <button type="button" className="add-alert-metric" onClick={addAlertMetric} disabled={alertForm.metrics.length >= alertMetrics.length}><IconPlus size={14} />添加监测指标</button>}</fieldset><label>检查频率<select value={alertForm.frequency} onChange={(event) => setAlertForm((form) => ({ ...form, frequency: event.target.value }))}><option>披露更新时</option><option>每日检查</option><option>每周检查</option><option>每月检查</option></select></label><div className="alert-form-note"><IconBell size={15} /><span>{editingAlertRuleId !== null ? `保存后将更新该指标规则，并继续监测已选的 ${alertForm.companyIds.length} 家企业。` : `将创建 ${alertForm.metrics.length} 条指标规则，并同时监测已选的 ${alertForm.companyIds.length} 家企业。`}</span></div><button className="alert-create-button" disabled={!alertForm.name.trim() || !alertForm.companyIds.length || alertForm.metrics.some((item) => !item.threshold)}>{editingAlertRuleId !== null ? <><IconCircleCheck size={16} />保存修改</> : <><IconPlus size={16} />创建 {alertForm.metrics.length} 条规则</>}</button></form>
  </div>;

  if (page === "alert-settings") return <main className="followed-workspace alert-page"><header className="alert-page-header"><button onClick={() => setPage("list")}><IconArrowLeft size={17} />返回关注企业</button><div><span>THRESHOLD SETTINGS</span><h1>阈值设置</h1><p>{alertSettingsMode === "form" || !alertRules.length ? "批量选择企业与监测指标，为每项指标单独设置触发条件。" : "查看并管理已设置的企业监测规则。"}</p></div><div className="alert-page-actions"><button onClick={() => setPage("alert-messages")}><IconBell size={16} />提醒消息</button>{alertRules.length > 0 && (alertSettingsMode === "form" ? <button onClick={() => { setEditingAlertRuleId(null); setAlertSettingsMode("list"); }}><IconArrowLeft size={15} />规则列表</button> : <button className="primary" onClick={openNewAlertRule}><IconPlus size={15} />新建提醒</button>)}</div></header><div className="alert-page-body">{alertRuleLayout}</div></main>;

  if (page === "alert-messages") return <main className="followed-workspace alert-page"><header className="alert-page-header"><button onClick={() => setPage("list")}><IconArrowLeft size={17} />返回关注企业</button><div><span>ALERT MESSAGES</span><h1>全部提醒消息</h1><p>查看关注企业触发的全部阈值提醒和数据更新。</p></div><button onClick={() => setPage("alert-settings")}><IconBell size={16} />规则设置</button></header><div className="alert-page-body"><section className="alert-message-list full-page"><div className="alert-message-toolbar"><strong>全部消息 · {alertMessages.length} 条</strong><button>全部标为已读</button></div>{alertMessages.map((message) => { const company = companies.find((item) => item.id === message.companyId); return <article className={message.unread ? "unread" : ""} key={message.id}><i><IconBell size={16} /></i><div><header><strong>{message.title}</strong>{message.unread && <b>未读</b>}<time>{message.time}</time></header><span>{company?.name}</span><p>{message.detail}</p></div><IconChevronRight size={16} /></article>; })}</section></div></main>;
  ...
  return <main className="followed-workspace"><header className="followed-header"><div><span>FOLLOWED ENTERPRISES</span><h1>{page === "select" ? "选择对比企业" : "关注的企业"}</h1><p>{page === "select" ? "请选择2–4家企业，确认后进入独立的对比详情页面。" : "集中查看已关注企业，并从企业列表发起横向对比。"}</p></div>{page === "list" && <div className="followed-header-actions"><button className="header-add-action" onClick={openAddCompanies}><IconPlus size={17} />添加企业</button><button className="header-alert-action" onClick={() => { setAlertSettingsMode("list"); setPage("alert-settings"); }}><IconBell size={17} />阈值设置{alertRules.length > 0 && <b>{alertRules.length}</b>}</button><button className="header-compare-action" onClick={startSelection}><IconChartBar size={17} />企业对比</button></div>}</header>
    <div className="followed-body">{alertMessages.length > 0 && <section className="followed-alert-preview"><header><div><IconBell size={16} /><strong>阈值提醒</strong><span>{alertMessages.filter((item) => item.unread).length} 条未读</span></div><button onClick={() => setPage("alert-messages")}>查看更多 <IconChevronRight size={14} /></button></header><div>{alertMessages.slice(0,5).map((message) => { const company = companies.find((item) => item.id === message.companyId); return <button key={message.id} onClick={() => setPage("alert-messages")}><i className={message.unread ? "unread" : ""} /><strong>{company?.short}</strong><span>{message.title}</span><time>{message.time}</time></button>; })}</div></section>}<section className="followed-list-toolbar"><div className="knowledge-search"><IconSearch size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索企业名称、证券代码或行业" /></div><div>{page === "select" ? <><span>已选择 {selected.length} / 4 家</span><button onClick={() => setPage("list")}>取消</button><button disabled={selected.length < 2} onClick={openComparison}>确认并开始对比 <IconArrowRight size={15} /></button></> : <><span>共 {filtered.length} 家</span><div className="view-toggle"><button className={viewMode === "cards" ? "active" : ""} onClick={() => setViewMode("cards")}><IconLayoutGrid size={15} /></button><button className={viewMode === "list" ? "active" : ""} onClick={() => setViewMode("list")}><IconListDetails size={15} /></button></div></>}</div></section>
      {filtered.length ? viewMode === "cards" && page === "list" ? <section className="followed-card-grid">{filtered.map((company) => { const activeAlertCount = alertRules.filter((rule) => rule.companyIds.includes(company.id) && rule.enabled).length; return <article key={company.id} onClick={() => onOpenDetail(company)}><header><i>{company.short.slice(0,1)}</i><div><strong>{company.name}</strong><small>统一社会信用代码 {company.creditCode || "—"}</small></div></header><div className="enterprise-card-tags">{(company.tags || []).slice(0,2).map((tag) => <span key={tag}>{tag}</span>)}{activeAlertCount > 0 && <span className="alert-chip"><IconBell size={11} />{activeAlertCount} 条提醒</span>}</div><dl className="enterprise-card-fields"><div><dt>行业</dt><dd>{company.industry}</dd></div><div><dt>股票类型</dt><dd>{companyStockType(company)}</dd></div><div><dt>股票代码</dt><dd>{company.code}</dd></div></dl><footer><span /><button className="card-more-action" aria-label={`取消关注${company.name}`} onClick={(event) => { event.stopPropagation(); onFollowChange(followedCompanies.filter((item) => item.id !== company.id).map((item) => item.id)); }}><IconDots size={19} /><em>取消关注</em></button></footer></article>; })}</section> : <section className={`followed-enterprise-list enterprise-fields-list ${page === "select" ? "selection-mode" : ""}`}><div className="head"><span>企业名称</span><span>企业标签</span><span>统一社会信用代码</span><span>行业</span><span>股票类型</span><span>股票代码</span><span /></div>{filtered.map((company) => { const checked = selectedIds.includes(company.id); return <article className={checked ? "selected" : ""} key={company.id}>{page === "select" && <button className="follow-check" onClick={() => toggleCompany(company.id)}>{checked ? <IconCircleCheck size={18} /> : <span />}</button>}<div className="followed-company-name"><i>{company.short.slice(0,1)}</i><div><strong>{company.name}</strong></div></div><div className="list-enterprise-tags">{(company.tags || []).slice(0,2).map((tag) => <span key={tag}>{tag}</span>)}</div><span>{company.creditCode || "—"}</span><span>{company.industry}</span><span>{companyStockType(company)}</span><span>{company.code}</span>{page === "select" ? <button className="select-row-hit" onClick={() => toggleCompany(company.id)} aria-label={`选择${company.name}`} /> : <button className="open-company" onClick={() => onOpenDetail(company)}>查看档案 <IconChevronRight size={14} /></button>}</article>; })}</section> : <section className="followed-empty"><IconBuilding size={28} /><strong>还没有关注企业</strong><p>在对话查询结果中点击“关注企业”，企业会自动出现在这里。</p></section>}
    </div>
...
function EnterpriseIntelView() {
  const [activeDimension, setActiveDimension] = useState("carbon");
  ...
  return <main className="enterprise-intel-shell">
    ...
    {fromFollowed && <nav className="intel-dimension-tabs"><button className={activeDimension === "carbon" ? "active" : ""} onClick={() => setActiveDimension("carbon")}><IconCloud size={16}/>碳排放</button><button className={activeDimension === "esg-reports" ? "active" : ""} onClick={() => setActiveDimension("esg-reports")}><IconFileDescription size={16}/>ESG报告</button></nav>}
    <section className={`intel-content ${fromFollowed ? "horizontal-dimension" : ""}`}>{!fromFollowed && <aside className="intel-side-nav"><span>数据维度</span><button className="active"><IconCloud size={18}/><div><strong>碳排放</strong><small>核算、趋势与目标</small></div><IconChevronRight size={15}/></button></aside>}<section className="intel-main-panel"><header className="intel-dimension-header"><div><h2>{activeDimension === "carbon" ? "碳排放" : "ESG报告"}</h2><p>{activeDimension === "carbon" ? dimensionMeta[2] : "查看企业历年可持续发展与环境、社会及治理报告。"}</p></div></header>{activeDimension === "carbon" || !fromFollowed ? <CarbonDimensionPanel/> : <div className="intel-dimension-body esg-report-page"><EsgReportTable/></div>}</section></section>
  </main>;
}
