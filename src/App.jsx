...
function CompanyListView({ followedCompanies, onOpenDetail, onFollowChange }) {
  ...
  const [alertRules, setAlertRules] = useState([
    { id: 1, companyIds: ["huaneng", "byd"], metric: "范围1+2排放强度", operator: "高于", threshold: "45", unit: "tCO₂e/MWh", frequency: "每月检查", enabled: true },
    { id: 2, companyIds: ["huaneng", "byd", "longi"], metric: "范围1+2排放总量", operator: "同比增长超过", threshold: "10", unit: "%", frequency: "披露更新时", enabled: true },
  ]);
  const [alertForm, setAlertForm] = useState({ companyIds: followedCompanies[0] ? [followedCompanies[0].id] : [], metrics: [{ metric: "范围1+2排放强度", operator: "高于", threshold: "", unit: "tCO₂e/MWh" }], frequency: "每月检查" });
  const alertMessages = [
  ...
  function openNewAlertRule() { setEditingAlertRuleId(null); setAlertForm((form) => ({ ...form, metrics: [{ metric: "范围1+2排放强度", operator: "高于", threshold: "", unit: "tCO₂e/MWh" }] })); setAlertSettingsMode("form"); }
  function editAlertRule(rule) { setEditingAlertRuleId(rule.id); setAlertForm({ companyIds: [...rule.companyIds], metrics: [{ metric: rule.metric, operator: rule.operator, threshold: rule.threshold, unit: rule.unit }], frequency: rule.frequency }); setAlertSettingsMode("form"); }
  function saveAlertRule(event) { event.preventDefault(); if (!alertForm.companyIds.length || alertForm.metrics.some((item) => !item.threshold)) return; if (editingAlertRuleId !== null) { const metric = alertForm.metrics[0]; setAlertRules((rules) => rules.map((rule) => rule.id === editingAlertRuleId ? { ...rule, ...metric, companyIds: [...alertForm.companyIds], frequency: alertForm.frequency } : rule)); } else { const now = Date.now(); const newRules = alertForm.metrics.map((metric, metricIndex) => ({ ...metric, companyIds: [...alertForm.companyIds], frequency: alertForm.frequency, id: now + metricIndex, enabled: true })); setAlertRules((rules) => [...rules, ...newRules]); } setEditingAlertRuleId(null); setAlertSettingsMode("list"); }
